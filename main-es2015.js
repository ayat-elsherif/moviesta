...
let InputCheckDirective = class InputCheckDirective {
    constructor(el) {
        this.el = el;
    }
    onBlur() {
        let myVal = this.el.nativeElement.value;
        this.el.nativeElement.value = myVal.toLowerCase();
    }
};
InputCheckDirective.ctorParameters = () => [
...
let MovieDetailsComponent = class MovieDetailsComponent {
    constructor(route, _SingleMovieService) {
        this.route = route;
...
let SingleMovieService = class SingleMovieService {
    constructor(_httpClient) {
        this._httpClient = _httpClient;
    }
    getSingleMediaType(media_type, id) {
...
SingleMovieService.ctorParameters = () => [
    { type: _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpClient"] }
];
